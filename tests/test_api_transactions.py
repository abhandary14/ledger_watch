...
class TestTransactionImportAPI:
    def test_valid_import_returns_201(self, auth_client, org):
        payload = {
            "transactions": [
                {
                    "date": "2026-01-10",
        ...
        response = auth_client.post(IMPORT_URL, payload, format="json")
        assert response.status_code == 201
        assert response.data["imported"] == 1

    def test_import_creates_transaction_in_db(self, auth_client, org):
        payload = {
            "transactions": [
                {"date": "2026-01-15", "vendor": "Stripe", "amount": "200.00"},
            ],
        }
        auth_client.post(IMPORT_URL, payload, format="json")
        assert Transaction.objects.filter(organization=org, vendor="Stripe").exists()

    def test_import_multiple_transactions(self, auth_client, org):
        payload = {
            "transactions": [
    ...
    def test_negative_amount_returns_400(self, auth_client, org):
        payload = {
            "transactions": [
    ...
    def test_invalid_date_format_returns_400(self, auth_client, org):
        payload = {
            "transactions": [
    ...
    def test_missing_vendor_returns_400(self, auth_client, org):
        payload = {
            "transactions": [
...
class TestTransactionListAPI:
    def test_returns_200(self, auth_client, transactions):
    ...
    def test_returns_paginated_results(self, auth_client, transactions):
        response = auth_client.get(LIST_URL)
        data = response.data
    ...
    def test_filter_by_vendor(self, auth_client, org):
        ...
        response = auth_client.get(LIST_URL, {"vendor": "FilterMe"})
        assert response.status_code == 200
        results = response.data["results"]
        assert len(results) >= 1
        assert all(tx["vendor"] == "FilterMe" for tx in results)

    def test_filter_by_category(self, auth_client, org):
        Transaction.objects.create(
            organization=org, date=date(2026, 1, 10),
        ...
        response = auth_client.get(LIST_URL, {"category": "Payroll"})
        assert response.status_code == 200
        results = response.data["results"]
        assert all(tx["category"] == "Payroll" for tx in results)

    def test_filter_by_date_from(self, auth_client, org):
        Transaction.objects.create(
        ...
        response = auth_client.get(LIST_URL, {"date_from": "2026-01-15"})
        assert response.status_code == 200
        results = response.data["results"]
        for tx in results:
            assert tx["date"] >= "2026-01-15"

    def test_filter_by_date_to(self, auth_client, org):
        Transaction.objects.create(
            organization=org, date=date(2026, 1, 5),
            vendor="Early", amount=Decimal("100.00")
        )
        ...
        response = auth_client.get(LIST_URL, {"date_to": "2026-01-10"})
        assert response.status_code == 200
        results = response.data["results"]
        for tx in results:
